...
from url_shortener_service.model import URL
from url_shortener_service.schema import UrlCreateResponse, UrlGetResponse, UrlUpdateResponse, UrlGetAllResponse
...
async def get_all_urls(db, user_id):
    """
...
async def create_new_url(db, user_id, url):
    ...
    """
    try:
        if UrlProcess.validate_url(url.value) is False: # validate the url
            return None
        id = await UrlProcess.shorten_url()

        while await get_url_by_id(db, id) is not None: # exist id
            id = await UrlProcess.shorten_url() # shorten the url
        db_url = URL(
            id=id,
            value=url.value,
...
async def url_of_valid_id(db, id):
    ...
    if not UrlProcess.is_valid_sha1_hash(id): # check id format
        return None
...
async def update_url_by_id(db, user_id, id, url):
    ...
    try:
        ...
        result = await db.execute(select(URL).filter(URL.id == id, URL.user_id == user_id))
        ...
        update_stmt = (
            update(URL)
            .where(URL.id == id, URL.user_id == user_id) # bonus: user can only update their urls
...
async def delete_urls(db, user_id):
    ...
    try:
        ...
        await db.execute(delete(URL).filter(URL.user_id == user_id))
        await db.commit()
    except Exception as e:
